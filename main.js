...
const dataURL = "http://api.exchangeratesapi.io/v1/latest?access_key=68cd1a066fee3ae630a56a28bb6b9f40";
...
function currenciesListClick(event) {
    if(event.target.classList.contains("close")) {
        ...
        if(parentNode.classList.contains("base-currency")) {
            const newBaseCurrencyListItem = currenciesList.querySelector(".currency");
            if(newBaseCurrencyListItem) {
                setNewBaseCurrency(newBaseCurrencyListItem);
                baseCurrencyAmount = Number(newBaseCurrencyListItem.querySelector(".input input").value);
            }
        }
    }
}
